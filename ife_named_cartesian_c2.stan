...
transformed data {
  ...
  // Loadings-prior width multiplier: scales the prior sd of every loading (and the inv_gamma
  // diagonal, whose beta scales identically so E[X^2] stays c^2/d). c = 1 reproduces the original.
  real lam_c = 2.0;
...
transformed parameters {
  ...
  // CARTESIAN equivalent of "unit-norm loadings + estimated sigma". Writing s[n] = sigma[n]*||Lambda[n,:]||
  // and u[n] = Lambda[n,:]/||Lambda[n,:]||, this model's mean is s[n]*(u[n].Phi) and -- with the error
  // scale below -- its error sd is tau*s[n]: algebraically the SAME model as the polar (unit-norm +
  // estimated sigma) version, but with the loadings in flat Cartesian coordinates.
  // Anchoring the error scale to the ESTIMATED signal scale s[n] (not the fixed sigma_data) is what keeps
  // tau inside its prior: otherwise tau is the only knob for extra error scale and gets dragged above it.
  vector<lower=0>[M_units] scale_est;      // s[n]: identified per-unit signal scale
  vector[M_units] lam_norm;
  for(m in 1:M_units) {
    lam_norm[m] = sqrt(dot_self(Lambda[m,:]));
    scale_est[m] = sigma[m] * lam_norm[m];
  }
  vector<lower=0>[M_units] error_precisions = inv(omega_sq * square(tau * scale_est));
  if(nonstationary) {
...
model {
  ...
  for(n in 1:M_units) {
    if(alpha_diag > 2 && n <= K_latent) {
      if(n > 1) {
        Lambda[n, 1:(n - 1)] ~ normal(0, lam_c / sqrt(n));
      }
      Lambda[n, n] ~ inv_gamma(alpha_diag, lam_c * sqrt((alpha_diag - 1) * (alpha_diag - 2) / n));
    } else {
      Lambda[n, 1:min(K_latent, n)] ~ normal(0, lam_c / sqrt(min(K_latent, n)));
    }
  }
