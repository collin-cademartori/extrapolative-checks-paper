...
fit1 <- function(ys, tru, arm, mult, a_rho, b_rho) {
  ...
  noise_abs <- as.numeric(cov(e_fit, noise) / var(noise))          # ~ effective df / n (all units)
  # RAW gen_gap is CIRCULAR: Y_latent includes delta, and true delta = 0 here, so the numerator
  # contains delta_hat -- the very thing absz measures. Remove it: Y_latent is linear in delta, so
  # posterior-mean counterfactual = fitm[post,1] - effect_means.
  gen_gap_raw <- rms(fitm[post, 1] - tru[post, 1]) / rms(e_fit)
  cf_hat <- fitm[post, 1] - f$effect_means
  gen_gap <- rms(cf_hat - tru[post, 1]) / rms(e_fit)               # de-circularised
  ovf <- rms(e_fit) / rms((fitm - ys)[cond])
  # restricted to the TREATED unit's pre-window: the actual extrapolation basis for the counterfactual
  pre <- seq_len(T_ - NPT)
  e_tr <- fitm[pre, 1] - tru[pre, 1]; n_tr <- ys[pre, 1] - tru[pre, 1]
  ovf_tr <- rms(e_tr) / rms(fitm[pre, 1] - ys[pre, 1])
  noise_abs_tr <- as.numeric(cov(e_tr, n_tr) / var(n_tr))
  rgh <- rms(diff(diff(fitm[seq_len(T_ - NPT), 1]))) / rms(diff(diff(tru[seq_len(T_ - NPT), 1])))
  # PLAIN error magnitudes on the treated unit's pre-window (the extrapolation basis).
  # mae_tru = how badly the fitted signal recovers TRUTH (basis inaccuracy)
  # mae_obs = in-sample residual; SMALL means the fit is interpolating the observations
  # scale-normalised versions (divided by sd of unit 1) are comparable across datasets, which matters
  # because absz is standardised while eabs is not.
  sd1 <- sd(ys[, 1])
  mae_tru <- mean(abs(e_tr)); mae_obs <- mean(abs(fitm[pre, 1] - ys[pre, 1]))
  structure(data.frame(arm = arm, mult = mult, rho_prior = sprintf("B(%g,%g)", a_rho, b_rho),
    ...
    noise_abs = round(noise_abs, 3), gen_gap = round(gen_gap, 2), gen_gap_raw = round(gen_gap_raw, 2),
    ovf = round(ovf, 3), ovf_tr = round(ovf_tr, 3), noise_abs_tr = round(noise_abs_tr, 3), rgh = round(rgh, 2),
    mae_tru = round(mae_tru, 3), mae_obs = round(mae_obs, 3),
    mae_tru_n = round(mae_tru / sd1, 3), mae_obs_n = round(mae_obs / sd1, 3), sd1 = round(sd1, 2),
    absz = round(mean(abs(f$effect_means / f$effect_sds)), 3), eabs = round(mean(abs(f$effect_means)), 3),
    div = f$sampler_diag$n_div, rhat = round(f$sampler_diag$rhat_max, 2)), fitm = fitm)
}
...
for (ds in study_units[1:3]) {
  ...
  for (a in arms) {
    ...
    ARR[[length(ARR) + 1]] <- list(ds = ds, arm = a[[1]], mult = a[[2]], rho_prior = sprintf("B(%g,%g)", a[[3]], a[[4]]),
      fitm = attr(r, "fitm"), tru = tru, ys = ys)
...
saveRDS(list(res = res, arrays = ARR), "/private/tmp/claude-502/-Users-collin-Documents-Wake-Research-PriorPredictivePaper-reprod/0edcfb95-808f-4867-8f30-15ec1e2aec59/scratchpad/ex1_metrics3.rds")
...
for (m in c("mae_tru_n", "mae_tru", "mae_obs_n", "mae_obs", "noise_abs_tr", "ovf_tr", "gen_gap", "noise_abs", "ovf", "rgh")) {
  cat(sprintf("  %-13s cor(.,absz)=%+.2f  cor(.,eabs)=%+.2f   [range %.2f - %.2f]\n",
    m, suppressWarnings(cor(s[[m]], s$absz)), suppressWarnings(cor(s[[m]], s$eabs)),
    min(s[[m]]), max(s[[m]]))) }
cat("\n=== WITHIN-ARM correlation (removes dataset-difficulty confounding) ===\n")
for (m in c("mae_tru_n", "mae_tru", "mae_obs_n", "noise_abs_tr", "ovf_tr", "gen_gap", "rgh")) {
  ws <- unlist(lapply(split(s, paste(s$arm, s$mult)), function(g)
    if (nrow(g) >= 3 && sd(g[[m]]) > 0 && sd(g$absz) > 0) cor(g[[m]], g$absz) else NA))
  cat(sprintf("  within-arm cor(%-12s, absz) = %s  (mean %+.2f)\n", m,
    paste(sprintf("%+.2f", ws), collapse=" "), mean(ws, na.rm = TRUE))) }
cat("\nby arm:\n")
