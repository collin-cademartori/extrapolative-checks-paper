...
source("../plotting.r")
# Optional CLI argument selects the results file: Rscript ex1_sim_study_summary.r sim_study_res.RData
.res_file <- commandArgs(trailingOnly = TRUE)[1]
if (is.na(.res_file)) .res_file <- "sim_study_ns.RData"
cat(sprintf("\nLoading %s\n", .res_file))
load(.res_file)

# Failed reps are reported before being excluded, so a study that lost reps
# is not summarized as though it were complete.
...
# Per-time mean and +/-2 SE bands for the nonstationary and stationary models, for
# either per-time statistic: the signed error (mean) or the standardized error (absz).
