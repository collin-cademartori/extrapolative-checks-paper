...
transformed parameters {
  ...
  // A/B VARIANT: error scale anchored to the FIXED sigma_data, NOT to sigma*||Lambda||.
  // scale_est is still computed above (for reporting) but does not enter the likelihood, so
  // error_precisions cannot blow up as a row norm shrinks. This is the original model's error
  // structure; the cost is that tau becomes the only knob for absolute error scale.
