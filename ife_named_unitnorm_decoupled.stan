...
transformed parameters {
  ...
  // DIAGNOSTIC VARIANT: decouple the error scale from the ESTIMATED amplitude. In the standard model the
  // SAME parameter (sigma) sets both the signal amplitude and the noise scale, so any move in sigma
  // rescales the fit AND the metric the fit is judged in -- the suspected divergence source. Here the
  // error scale uses the FIXED sigma_data (the signal still uses the estimated sigma).
  vector<lower=0>[M_units] error_precisions = inv(omega_sq * square(tau * sigma_data));
  if(nonstationary) {
