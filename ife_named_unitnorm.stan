...
model {
  ...
  // Loadings prior. Under the unit-norm reparameterization the *direction* of each row
  // is what enters the outcome (sigma[n] carries the magnitude), so the prior goes on the
  // raw Cholesky factor: a weak std_normal that properly identifies the row magnitude and
  // induces a ~uniform direction. Priors must NOT be placed on the transformed (normalized)
  // Lambda -- that needs a manual Jacobian, and the magnitude is non-identified without this.
