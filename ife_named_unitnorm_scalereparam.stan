...
transformed data {
  ...
  // Orthonormal basis of the sum-to-zero subspace (Stan User's Guide, "Parameterizing Centered
  // Vectors", QR construction). log_sigma_dev = sumzero_Q * log_sigma_dev_raw then sums to zero for
  // any raw vector; sumzero_Q has orthonormal columns so the map is isometric (constant Jacobian).
  matrix[M_units, M_units - 1] sumzero_Q;
  {
    matrix[M_units, M_units] A = diag_matrix(rep_vector(1.0, M_units));
    for(i in 1:(M_units - 1)) {
      A[M_units, i] = -1;
    }
    A[M_units, M_units] = 0;
    sumzero_Q = qr_Q(A)[ , 1:(M_units - 1)];
  }
...
parameters {
  ...
  // Global-scale reparameterization of the per-unit scales. The likelihood only sees
  // sigma[n] * (Lambda[n] . Phi), so every sigma[n] trades off against the factors' realized
  // amplitude together -- making the sigma_raw collinear (r up to 0.99) and the geometry a
  // diagonal ridge the diagonal metric cannot stretch. Split that: log_sigma_g carries the
  // shared (weakly identified) common mode, and sum-to-zero log_sigma_dev the identified per-unit
  // differences, so the shallow direction lies along a single coordinate axis.
  array[fit_overall_scales == 1 ? 1 : 0] real log_sigma_g;
  // Sample the deviations UNCONSTRAINED (length M-1); sum_to_zero_constrain() maps them to a zero-sum
  // length-M vector in transformed parameters. Keeps the clean common-mode removal of a
  // sum_to_zero_vector while leaving a plain vector that pathfinder can seed. The transform is
  // orthonormal (volume-preserving), so no Jacobian adjustment is required.
  vector[fit_overall_scales == 1 ? M_units - 1 : 0] log_sigma_dev_raw;
...
transformed parameters {
  ...
  if(fit_overall_scales == 1) {
    sigma = sigma_data .* exp(log_sigma_g[1] + sumzero_Q * log_sigma_dev_raw);
  } else {
    sigma = sigma_data;
  }
...
model {
  ...
  if(fit_overall_scales == 1) {
    log_sigma_g[1] ~ normal(0, 1.5);       // shared log-amplitude: weakly identified, the long axis
    // raw scaled so the induced zero-sum deviations have ~unit marginal sd (Q orthonormal =>
    // Var(dev_i) = s^2 * (1 - 1/M); s = inv_sqrt(1 - 1/M) gives unit marginal variance).
    log_sigma_dev_raw ~ normal(0, inv_sqrt(1 - inv(M_units)));
    // Induced marginal on the multiplier exp(log_sigma_g + dev) is lognormal, median 1, wide
    // (~half-normal(0,5) scale), so sigma is centred on the data scale sigma_data.
  }
