...
ROWDIR <- file.path(SP, "ex2_science_orig_rows"); dir.create(ROWDIR, showWarnings = FALSE)
...
fit_one <- function(row) {
  ...
  tryCatch({
    ...
    secs <- system.time(f <- do.call(sample_model, c(list(
      ...
      fit_scales = 0, alpha_diag = 10, pathfinder_init = TRUE, type = "posterior", quiet = TRUE,
...
save(res, file = file.path(SP, "ex2_science_orig.RData"))
res <- res[order(res$model, res$sim, res$N_comp, res$rep), ]
cat("\nex2 unit-norm ig est-sigma | SCIENCE on ORIGINAL model (fixed sigma, non-unit-norm) | both models | sim{.7,.9} x N_comp{2,3} x2 reps | ad=0.95\n\n")
