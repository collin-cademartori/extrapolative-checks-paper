...
ife_mod <- cmdstan_model("../ife_named_unitnorm_decoupled.stan"); stopifnot("Lambda_raw" %in% PF_PARAM_BASES)
...
run <- function(tau_val, label) {
  ...
  sig <- sapply(1:N_, function(n) as.numeric(extract_variable_matrix(d, sprintf("sigma[%d]",n))))
  rh <- tryCatch(max(f$summary(c("delta","sigma"),"rhat")$rhat,na.rm=TRUE), error=function(e) NA)
  cat(sprintf("%-26s div=%-5d tree=%-4d td_mean=%.1f rhat=%.2f | sd(log tau)=%s | mean sigma=%.2f | %.0f s\n",
    label, sum(dg$num_divergent), sum(dg$num_max_treedepth), mean(td), rh,
    ifelse(is.na(sd_logtau),"fixed",sprintf("%.3f",sd_logtau)), mean(colMeans(sig)),
    as.numeric(difftime(Sys.time(), t0, units="secs"))))
}
cat("\n=== ERROR-SCALE DECOUPLING intervention (unit-norm, sigma FREE in both arms, ad=0.9, 6 chains) ===\n")
cat("baseline for reference: unitnorm+est tau-free previously gave div=408, tree=138\n\n")
run(0, "DECOUPLED err scale")
