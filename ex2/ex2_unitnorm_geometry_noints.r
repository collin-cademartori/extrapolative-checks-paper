...
secs <- system.time(f <- sample_model(N_units = N_, T_times = nrow(fit_ys), K_latent = K_LAT,
  overall_scales = apply(fit_ys, 2, function(x) sqrt(mean(x^2))), err_scale = 0, err_scale_mean = 0.1, err_scale_sd = 0.05,
  data = fit_ys, autocor_a = 90, autocor_b = 10, nonstationary = FALSE, num_treated = NPT,
  include_factor_means = TRUE, fit_scales = TRUE, alpha_diag = 10,
  ...
  return_draws = c("lp__", "sigma_raw", "sigma", "tau", "Lambda_raw", "delta_raw", "rho")))["elapsed"]
d <- f$draws
sd_ <- f$sampler_diag
...
saveRDS(list(agg = agg, ss = ss, secs = secs, td = td, leap = leap), file.path(SP, "ex2_geometry_noints.rds"))
