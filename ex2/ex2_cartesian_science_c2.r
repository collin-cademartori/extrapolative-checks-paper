...
## Original ex2 SCIENCE check for the Cartesian model (ife_named_cartesian_c2.stan): does it reproduce the two ex2
...
ife_mod <- cmdstan_model("../ife_named_cartesian_c2.stan")
stopifnot(basename(ife_mod$stan_file()) == "ife_named_cartesian_c2.stan", "Lambda" %in% PF_PARAM_BASES)
...
ROWDIR <- file.path(SP, "ex2_cartc2_rows"); dir.create(ROWDIR, showWarnings = FALSE)
...
save(res, file = file.path(SP, "ex2_cartesian_science_c2.RData"))
res <- res[order(res$model, res$sim, res$N_comp, res$rep), ]
cat("\nex2 CARTESIAN model | loadings prior c=2 | science check | ad=0.8, 4 chains | both models\n")
