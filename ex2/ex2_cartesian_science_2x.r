## ex2 SCIENCE check, sigma_data DOUBLED (2x overall_scales) -- the candidate ex1 fix.
## Predictions: cor_sq is SCALE-INVARIANT (numerator and denominator both scale by the same factor) so
## the gap should survive; tau ~ N/S so it halves IF the prior pins ||Lambda|| (mult stays ~0.9) but is
## unchanged if the data pin S (mult halves to ~0.47). mult is the readout of which wins.
## Original ex2 SCIENCE check for the Cartesian model (ife_named_cartesian.stan): does it reproduce the two ex2
...
GRID <- expand.grid(rep = 1, N_comp = c(2, 3), sim = c(0.7, 0.9), model = c("no_ints", "ints"), stringsAsFactors = FALSE)
...
ROWDIR <- file.path(SP, "ex2_cart2x_rows"); dir.create(ROWDIR, showWarnings = FALSE)
...
fit_one <- function(row) {
  ...
  tryCatch({
    ...
    # DOUBLED sigma_data: does the 2x scale (candidate ex1 fix) undo the ex2 science?
    os <- 2 * (if (ints) apply(fit_ys, 2, sd) else apply(fit_ys, 2, function(x) sqrt(mean(x^2))))
...
save(res, file = file.path(SP, "ex2_cartesian_science_2x.RData"))
res <- res[order(res$model, res$sim, res$N_comp, res$rep), ]
cat("\nex2 CARTESIAN model | sigma_data DOUBLED | science check | ad=0.8, 4 chains | both models\n")
