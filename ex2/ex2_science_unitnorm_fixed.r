...
ROWDIR <- file.path(SP, "ex2_science_unfix_rows"); dir.create(ROWDIR, showWarnings = FALSE)
...
save(res, file = file.path(SP, "ex2_science_unfix.RData"))
res <- res[order(res$model, res$sim, res$N_comp, res$rep), ]
cat("\nex2 unit-norm ig est-sigma | SCIENCE confirmation | UNIT-NORM + FIXED sigma | sim{.7,.9} x N_comp{2,3} x2 reps | ad=0.95\n\n")
